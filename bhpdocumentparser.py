...
class BhpDocumentParser:
    ...
    @staticmethod
    def getChildElementByTagName(elem, name):
    ...
    # Returns tuple (crsattribute, x, y)
    def getCrsAndLocation(self):
        elem = self.getElementByTagName('location')
        if (elem is not None):
            child = BhpDocumentParser.getChildElementByTagName(elem, 'pos')
            if child is not None:
                x, y = child.text.split(" ")
                crs = elem.attrib['srsName']
                return (crs , float(x), float(y))
        return None
    
    # Returns x, y in the EPSG:28992 coordinate system
    def getLocationStandardized(self):
        result = self.getCrsAndLocation()
        if (result is not None):
            (crs, x, y) = result
            epsg = re.sub('.*EPSG.', 'EPSG', crs)
            if (epsg == 'EPSG:28992'):
                return x, y
            p1 = Proj(init=epsg)
            p2 = Proj(init='EPSG:28992')
            # position in document is lon/lat (y, x) for latlong projection
            # while position in document is x, y when not
            if (p1.is_latlong()):
                return transform(p1, p2, y, x)
            else:
                return transform(p1, p2, x, y)
        return None
